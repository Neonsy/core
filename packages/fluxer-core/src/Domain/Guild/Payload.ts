...
/** Top-level snapshot fields that Fluxer keeps outside `properties`. */
...
/** Copy Fluxer top-level count fields onto a nested `properties` guild object. */
function mergeTopLevelSnapshotFields(
 ...
 *
 * Fluxer puts `member_count` / `online_count` on the snapshot root (alongside `properties`),
 * not inside `properties` — those are merged onto the normalized guild object.
 */
