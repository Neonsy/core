...
import { toRoleRequestBody, type RoleCreateOptions } from '../roleOptions.js';
...
export async function fetchRole(guild: Guild, roleId: string): Promise<Role> {
  try {
    ...
    rethrowNotFound(
      err,
      { code: ErrorCodes.RoleNotFound, message: `Role ${roleId} not found in guild` },
      'Failed to fetch guild role',
    );
  }
}
