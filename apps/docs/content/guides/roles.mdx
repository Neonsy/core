...
Use `member.roles.add()`, `member.roles.remove()`, and `member.roles.set()`. `member.roles.cache` is a Collection of Role objects. Resolve or fetch the member first when you only have a user ID  -  `guild.addRoleToMember` / `removeRoleFromMember` were removed.
