...
Builders (`EmbedBuilder`, etc.) throw `RangeError` for fixed embed text and field-count limits. They reject invalid HTTP(S) and attachment URLs, required strings that normalize to empty, and malformed runtime input. Builder input is not truncated. Configured message-content, embed-count, and aggregate limits are reported by the API.
