...
The default guild limit is 200. A guild count that remains exactly at that value can mean older guilds are being FIFO-evicted. Set `cache.guilds: 0` for an unbounded guild cache, or choose a limit that fits the bot's workload.
...
const messageGuild = message.guild ?? (await message.resolveGuild());
// null means the message is a DM. REST failures reject with their original context.
```
...
client.cache.stats();
// {
//   guilds, channels, users, members, messages, messageChannels,
//   roles, emojis, stickers,
//   evictions: { guilds, channels, users, members, messages, messageChannels, roles, emojis, stickers }
// }
...
| Symptom | Check |
| ------- | ----- |
| Guild count remains exactly at 200, or `message.guild` is unexpectedly `null` | Check `client.cache.limits.guilds` and `client.cache.stats().evictions.guilds`. Use `await message.resolveGuild()` when the message can outlive the cached guild. |
