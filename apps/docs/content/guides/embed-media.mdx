...
`setVideo` / `setAudio` are removed. Those fields are response-only when reading embeds. See [Migrating to 2.0](/guides/migration/).

## GIFs in embeds

Embeds need a GIF URL for animated images (not MP4). For Tenor, use `resolveTenorToImageUrl()` — see [GIFs (Tenor)](/guides/gifs/).
