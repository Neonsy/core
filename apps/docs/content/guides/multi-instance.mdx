...
description: "Run bots against hosted Fluxer, a self-hosted instance, or both in the same Node process. One Client per instance. ClientCluster (beta) can add/remove runtimes without restart."
...
To enable one diagnostics policy for every cluster-created client and collect one report, see
[Diagnostics](/guides/diagnostics/#clientcluster).
