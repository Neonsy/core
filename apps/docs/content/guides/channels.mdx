...
Create roles with guild.createRole(), fetch with guild.fetchRoles() or guild.fetchRole(roleId). Add/remove with member.roles.add() / member.roles.remove(). Reorder with guild.setRolePositions().
...
| `Roles` | `member.roles.add()` | Add role to member |
| `Roles` | `member.roles.remove()` | Remove role from member |
