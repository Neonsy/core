...
`message.reply()` posts a threaded reply to the triggering message. `message.send()` posts in the same channel with no reference: a normal standalone message.
...
Use `message.sendTo(channelId, payload)` to post elsewhere (logs, announcements, forwarding). You only need the target channel ID.
