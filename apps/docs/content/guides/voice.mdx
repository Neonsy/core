---
title: Voice
description: Join a voice channel and play WebM/Opus audio with @fluxerjs/voice.
category: voice
order: 14
---

<Warning>
**Voice is being reworked.** Fluxer is updating how voice works, so `@fluxerjs/voice` and the demos below will change soon. Treat this guide as temporary. Do not build production bots on the current voice APIs.
</Warning>
...
The [voice-bot](/examples/voice-bot/) example implements `!play`, `!playvideo`, and `!stop`. Same rework warning applies there. Enable [Diagnostics](/guides/diagnostics/) to capture bounded voice lifecycle and media-pipeline events.

Also useful: [Installation](/guides/installation/), [Basic bot](/guides/basic-bot/), [Events](/guides/events/).
